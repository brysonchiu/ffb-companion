...
import { OverallList } from './components/player/OverallList.jsx';
import { PositionalLists } from './components/player/PositionalLists.jsx';
import { Settings } from './components/Settings.jsx';
import { MobileMessage } from './components/MobileMessage.jsx';
...
function App() {
  ...
  const [draftStatus, setDraftStatus] = useState(() => (localStorage.getItem('draftStatus') ? JSON.parse(localStorage.getItem('draftStatus')) : {}));
  const [sentimentStatus, setSentimentStatus] = useState(() =>
    localStorage.getItem('sentimentStatus') ? JSON.parse(localStorage.getItem('sentimentStatus')) : {},
  );
  const [playerSearch, setPlayerSearch] = useState('');
  const [settings, setSettings] = useState(() => {
    if (localStorage.getItem('settings')) {
      return compareObjectsProperties(defaultSettings, JSON.parse(localStorage.getItem('settings')))
  ...
  const [currentPick, setCurrentPick] = useState(() => (localStorage.getItem('currentPick') ? JSON.parse(localStorage.getItem('currentPick')) : 1));
